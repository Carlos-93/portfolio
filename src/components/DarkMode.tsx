...
export default function DarkMode() {
    ...
    return (
        <>
            {/* Global toast container — lives here to reuse the isDark state for theming */}
            <Toaster theme={isDark ? 'dark' : 'light'} richColors position="top-center" closeButton offset={96} mobileOffset={80} />

            <button type="button" data-theme-toggle onClick={switchTheme} aria-label={isDark ? t('common.switchToLight') : t('common.switchToDark')}
                className={`fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-10 cursor-pointer inline-flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-full border transition duration-300 ease-in-out hover:scale-105 backdrop-blur-xs 
            ${isDark ? 'border-slate-200/40 bg-slate-900/80 text-slate-200' : 'border-slate-900/40 bg-white/85 text-slate-900/80'}`}>
