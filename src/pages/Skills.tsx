                                ...
                                {items.map((skill) => (
                                    <li key={skill.name}>
                                        <a href={skill.url} target="_blank" rel="noopener noreferrer"
                                            ...
                                            className="group relative flex h-full flex-col items-center justify-center gap-3 p-6 rounded-2xl border border-slate-200 bg-white/50 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:border-cyan-500/60 hover:shadow-lg hover:shadow-cyan-500/20 dark:border-white/10 dark:bg-white/5 sm:gap-4">
                                            ...
                                            <img src={`/assets/icons/${skill.image}`} alt={skill.name} loading="lazy"
                                                className={`h-12 w-12 transition-transform duration-300 group-hover:scale-110 sm:h-14 sm:w-14${skill.invertOnDark ? ' dark:invert' : ''}`}
                                            />
